...
function cardInnerHtml(project, t) {
  return `
    ...
    <div class="flex flex-wrap gap-1.5 mb-3">
      ${project.technologies
        .map(
          (tech, idx) =>
            `<span class="text-[10px] text-blue-100/90 bg-white/10 px-2.5 py-1 rounded border border-white/20 hover:bg-white/20 hover:border-white/35 transition-all duration-200">${t(`projects.data.${project.id}.technologies.${idx}`, { defaultValue: tech })}</span>`
        )
        .join('')}
    </div>
...
export function renderProjectsSectionHtml(i18n) {
  const t = (k, opts) => i18n.t(k, opts);

  if (projectsData.length === 0) {
    return `
      <section id="projects" class="liquid-section py-20 px-4 bg-bg-secondary/20">
  ...
  const cards = projectsData
    .map(
      (project) => `
    <div class="spotlight-card relative group liquid-panel liquid-panel-interactive glass-blur-strong rounded-lg p-5 glass-border transition-all duration-300 flex flex-col h-full">
      <div class="spotlight-overlay"></div>
      ${cardInnerHtml(project, t)}
    </div>
  `
    )
    .join('');
  ...
  return `
    <section id="projects" class="liquid-section py-20 px-4 bg-bg-secondary/20" data-ssr-projects-grid>
      <div class="max-w-5xl mx-auto">
        <header class="text-center mb-16">
          <div class="flex items-center justify-center gap-4 mb-6">
            <h2 class="text-3xl font-light liquid-title tracking-wide animate-fadeInUp">${t('projects.title')}</h2>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4 mb-8">${cards}</div>
      </div>
    </section>
  `;
}
