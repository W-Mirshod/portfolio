import { iconArrowRight } from '../../components/ui/Icons.js';

export function renderHomeSectionHtml(i18n) {
  const t = (k) => i18n.t(k);
  ...
  const staticBg = `
    <div class="absolute inset-0 w-full h-full overflow-hidden pointer-events-none">
      ...
      <div class="absolute inset-0 w-full h-full bg-gradient-to-t from-transparent via-transparent to-bg-secondary/10"></div>
    </div>
  `;
        ...
        <div class="flex flex-col items-center justify-center text-center w-full space-y-4 sm:space-y-5 md:space-y-6 px-4 sm:px-6 md:px-8 lg:px-12 xl:px-16 lg:items-start lg:text-left">
          <div class="flex flex-col sm:flex-row items-center justify-center gap-4 sm:gap-6 layered-entrance lg:justify-start">
            <div class="w3d-hero-wrapper">
              <div class="w3d-hero-canvas" data-w3d-hero>
                <div class="w-full h-full rounded-full border-4 border-white/35 shadow-2xl ring-2 ring-white/40 liquid-panel-strong flex items-center justify-center brand-wmark brand-wmark-hero">M<span class="brand-wmark-dot">.</span></div>
              </div>
            </div>
          ...
          <div class="home-buttons flex flex-wrap items-center justify-center gap-3 sm:gap-4 mt-6 sm:mt-8 layered-entrance lg:justify-start" style="animation-delay:0.3s">
            <button type="button" data-hydrate-home-nav="#projects" data-i18n-btn="home.buttons.viewProjects" class="magnetic-button transition-transform duration-300 ease-out liquid-btn px-6 sm:px-8 lg:px-10 py-3 sm:py-3.5 lg:py-4 text-white font-medium text-sm sm:text-base lg:text-lg">${t('home.buttons.viewProjects')}</button>
            <button type="button" data-hydrate-home-nav="#contact" data-i18n-btn="home.buttons.letsTalk" class="magnetic-button transition-transform duration-300 ease-out liquid-btn liquid-btn-primary px-6 sm:px-8 lg:px-10 py-3 sm:py-3.5 lg:py-4 font-medium text-sm sm:text-base lg:text-lg">${t('home.buttons.letsTalk')}</button>
          </div>
        </div>
  ...
  return `
    <section id="home" class="liquid-section section-accent-glow relative flex flex-col items-center justify-center min-h-screen w-full overflow-hidden pt-16 sm:pt-20 md:pt-24 lg:pt-10 px-3 sm:px-4 md:px-6 lg:px-8 xl:px-20">
      ${staticBg}
      ${inner}
    </section>
  `;
