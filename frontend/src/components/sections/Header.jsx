  ...
  return (
    <>
      <aside className="hidden xl:fixed xl:flex flex-col items-center top-0 right-0 h-full z-30 desktop-sidebar transition-all duration-300 w-28 sm:w-32" style={{fontFamily:'Poppins,sans-serif',letterSpacing:'0.03em'}} aria-label="Main navigation">
        <div className="w-full h-full liquid-panel-strong border-l border-white/20 flex flex-col items-center relative" style={{transform: 'translateZ(0)', willChange: 'backdrop-filter'}}>
          <div className="relative z-10 flex flex-col h-full w-full items-center">
            <a href="#home" className="flex flex-col items-center gap-2 mt-12 mb-4 group" onClick={(e) => {
              ...
              <div className="rounded-xl liquid-panel border border-white/30 p-1 transition-all duration-300 group-hover:scale-105 w-14 h-14 sm:w-16 sm:h-16">
            ...
            <div className="mt-auto mb-6 flex flex-col items-center gap-1.5">
              ...
              <button
                onClick={toggleTheme}
                className="theme-toggle-btn w-20 h-12 sm:w-24 sm:h-14 flex items-center justify-center rounded-xl liquid-panel border border-white/20 hover:border-white/40 transition-all duration-300 group"
              ...
              <div className="relative language-dropdown-container">
                <button
                  className="w-20 h-12 sm:w-24 sm:h-14 flex items-center justify-center liquid-panel border border-white/20 rounded-xl font-semibold text-sm text-white hover:bg-white/20 hover:border-white/40 transition-all duration-300"
                  aria-haspopup="true"
                ...
                {isLanguageOpen && (
                  <div className="absolute bottom-full left-full ml-2 mb-2 liquid-panel-strong border border-white/40 rounded-xl overflow-hidden min-w-[110px] z-[100]" role="menu" aria-label="Language selection">
                    {languages.map((language) => (
                      <button
                        key={language.code}
                        className={`w-full px-4 py-2.5 text-left text-sm transition-all duration-200 ${currentLanguage === language.code ? 'bg-white/25 text-white border-l-2 border-white/80' : 'text-white/80 hover:bg-white/15 hover:text-white'}`}
      ...
      <header className="fixed top-0 left-0 w-full z-20 xl:hidden mobile-header" style={{letterSpacing: '0.03em', transform: 'translateZ(0)', willChange: 'backdrop-filter'}} role="banner">
        <div className="w-full liquid-panel-strong border-b border-white/20 relative">
          <div className="relative z-10">
            ...
            <div className="flex items-center gap-2 sm:gap-3">
              <a href="#home" className="flex items-center gap-2 sm:gap-3 group" aria-label="Go to home section">
                <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-lg liquid-panel border border-white/30 p-1 transition-all duration-300 group-hover:scale-105">
                  <img src={MirshodImg} alt="W" className="w-full h-full rounded object-cover" />
                </div>
            ...
            <div className="relative flex items-center gap-2 sm:gap-3">
              ...
              <button
                onClick={toggleTheme}
                className={`theme-toggle-btn w-8 h-8 sm:w-10 sm:h-10 flex items-center justify-center rounded-lg liquid-panel border border-white/20 hover:border-white/40 transition-all duration-300 group ${isMenuOpen ? 'hidden' : ''}`}
                aria-label={`Switch to ${(theme || 'dark') === 'dark' ? 'light' : 'dark'} mode`}
                title={`Switch to ${(theme || 'dark') === 'dark' ? 'light' : 'dark'} mode`}
              >
              ...
              <div className="relative language-dropdown-container">
                <button
                  className="px-3 sm:px-4 py-2 liquid-panel border border-white/20 rounded-lg font-semibold text-xs sm:text-sm text-white hover:bg-white/20 hover:border-white/40 transition-all duration-300"
                  aria-haspopup="true"
                  aria-expanded={isLanguageOpen}
                  aria-label={`Current language: ${getLanguageDisplay(currentLanguage)}. Click to change language`}
                  onClick={() => setIsLanguageOpen(!isLanguageOpen)}
                ...
                {isLanguageOpen && (
                  <div className="absolute top-full right-0 mt-2 liquid-panel-strong border border-white/40 rounded-xl overflow-hidden min-w-[100px] z-50" role="menu" aria-label="Language selection">
                    {languages.map((language) => (
                      <button
                        key={language.code}
                        className={`w-full px-3 py-2 text-left text-sm transition-all duration-200 ${currentLanguage === language.code ? 'bg-white/25 text-white border-l-2 border-white/80' : 'text-white/80 hover:bg-white/15 hover:text-white'}`}
                        onClick={() => handleLanguageChange(language.code)}
                        role="menuitem"
                        aria-label={`Switch to ${language.name}`}
                        aria-selected={currentLanguage === language.code}
              ...
              <div
                className="flex flex-col justify-center items-center w-8 h-8 sm:w-10 sm:h-10 liquid-panel border border-white/20 rounded-lg cursor-pointer transition-all duration-300 hover:scale-105 hover:bg-white/20"
                role="button"
                tabIndex={0}
        ...
        {typeof window !== 'undefined' && isMenuOpen && createPortal(
          <>
            <nav className="fixed top-0 left-0 h-screen w-[280px] sm:w-[320px] liquid-panel-strong border-r border-white/20 z-50 flex flex-col gap-2 sm:gap-3 px-4 sm:px-5 py-6 relative overflow-hidden animate-slideInLeft" id="mobileNavMenu" style={{letterSpacing:'0.03em', transform: 'translateZ(0)', willChange: 'backdrop-filter'}} aria-label="Mobile navigation menu">
              <div className="relative z-10 flex flex-col h-full">
                <a href="#home" className="flex flex-col items-center gap-2 sm:gap-3 mb-4 sm:mb-6 group" onClick={(e) => {
                  e.preventDefault();
                  handleNavigation('#home');
                }} aria-label="Go to home section">
                  <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-lg liquid-panel border border-white/30 p-1 transition-all duration-300 group-hover:scale-105">
                    <img src={MirshodImg} alt="W" className="w-full h-full rounded object-cover" />
                  </div>
                ...
                <div className="flex justify-center mb-4">
                  <button
                    onClick={toggleTheme}
                    className="theme-toggle-btn w-12 h-12 flex items-center justify-center rounded-xl liquid-panel border border-white/20 hover:border-white/40 transition-all duration-300 group"
                    aria-label={`Switch to ${(theme || 'dark') === 'dark' ? 'light' : 'dark'} mode`}
                    title={`Switch to ${(theme || 'dark') === 'dark' ? 'light' : 'dark'} mode`}
                  >
                    <div className="relative w-5 h-5">
